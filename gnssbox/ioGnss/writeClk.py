...
def writeClk(clkHead, clkData, clkFile):
    ...
    # 写入头文件
    # 版本号，类型
    clkFileWrite.write('     ' + '%.2f' % clkHead['RINEX VERSION'] + '           C                   M                   RINEX VERSION / TYPE\n')
    # 内容，组织
    clkFileWrite.write(str(clkHead['PGM']).ljust(20) + str(clkHead['RUN BY']).ljust(20) + '                    PGM / RUN BY / DATE \n')
    # 判断钟差写入类型
    if len(clkHead['TYPES OF DATA']) == 0:
        ...
        clkFileWrite.write('     1    ' + clkHead['TYPES OF DATA'][0] + '                                                # / TYPES OF DATA   \n')
    elif len(clkHead['TYPES OF DATA']) == 2:
        clkFileWrite.write('     2    AR    AS                                          # / TYPES OF DATA   \n')
    # 时间格式
    clkFileWrite.write('   GPS                                                      TIME SYSTEM ID \n')
    # 'INTERVAL'
    if clkHead['INTERVAL'] != None:
        clkFileWrite.write(str('%.2f' % clkHead['INTERVAL']).rjust(9) + '                                                   INTERVAL\n')
    # 组织单位
    clkFileWrite.write(str(clkHead['ANALYSIS CENTER']).ljust(60) + 'ANALYSIS CENTER\n')
    # 卫星数量
    clkFileWrite.write(str(len(clkHead['SOLN SATS'])).rjust(6) + '                                                      # OF SOLN SATS      \n')
    # PRN LIST
        ...
        for prn in clkData['sat']:
            for prnData in clkData['sat'][prn]:
                if prnData.epoch == epoch:
                    if prnData.clkVel == None:
                        line = 'AS ' + str(prn).ljust(4) + ' ' + str(epoch.year) + ' ' + str(epoch.month).zfill(2) + ' ' + str(epoch.day).zfill(2) + ' ' + str(epoch.hour).zfill(2) + ' ' + \
                            str(epoch.minute).zfill(2) + ('%.6f' % epoch.second).rjust(10) + '  1' + ('%.12E' % prnData.clk).rjust(22)
                        
                    else:
                        line = 'AS ' + str(prn).ljust(4) + ' ' + str(epoch.year) + ' ' + str(epoch.month).zfill(2) + ' ' + str(epoch.day).zfill(2) + ' ' + str(epoch.hour).zfill(2) + ' ' + \
                            str(epoch.minute).zfill(2) + ('%.6f' % epoch.second).rjust(10) + '  2' + ('%.12E' % prnData.clk).rjust(22) + ('%.12E' % prnData.clkVel).rjust(20)
                    clkFileWrite.write(line + '\n')
                    break